...
Fixes BOTH oracle failures:
  - gamma(0.001) at 10 ULP
  - lgamma(0.1) at 10 ULP

Root cause:
  The 8-step recurrence for x < 0.5 evaluates Stirling at x+8.
  At x+8 ≈ 8, the Stirling correction (~0.0104) is still large
  enough that its double-precision rounding error survives into
  the DD chain and reaches ~10 ULP after exp().

Fix:
  Increase recurrence depth from 8 to 16 for x < 0.5.
  Stirling is now evaluated at x+16 where the correction is
  ~0.0052 (half as large), halving the injected error.
...
def patch_integral(v12, force):
    ...
    if MARKER in text and not force:
        ...
        return
    count = 0
    if OLD_GAMMA in text:
        text = text.replace(OLD_GAMMA, NEW_GAMMA, 1)
        count += 1
    if OLD_LGAMMA in text:
        text = text.replace(OLD_LGAMMA, NEW_LGAMMA, 1)
        count += 1
    if count == 0:
...
def main():
    ...
    print(f"  Force: {force}")
    print("---------------------------------------------------------")
    print("\n[1/1] integral.c — recurrence depth 8 -> 16")
    ...
    print("    - Both gamma and lgamma paths updated")
    print("")
    print("  Verify:")
    print("    cd v12A1")
