    ...
    for (size_t pn = 0; pn < 2; pn++) {
        for (size_t pc = 0; pc < 3; pc++) {
            for (size_t ph = 0; ph < 4; ph++) {
                for (size_t pw = 0; pw < 5; pw++) {
                    dummy_in[((pn * 3 + pc) * 4 + ph) * 5 + pw] =
                        (double)(pn * 1000 + pc * 100 + ph * 10 + pw);
                }
            }
        }
    }
    ...
    if (perm->runtime_data != NULL) {
        ...
        for (size_t pn = 0; pn < 2 && perm_pass; pn++) {
            for (size_t ph = 0; ph < 4 && perm_pass; ph++) {
                for (size_t pw = 0; pw < 5 && perm_pass; pw++) {
                    for (size_t pc = 0; pc < 3 && perm_pass; pc++) {
                        size_t out_idx = ((pn * 4 + ph) * 5 + pw) * 3 + pc;
                        size_t in_idx = ((pn * 3 + pc) * 4 + ph) * 5 + pw;

                        if (perm->runtime_data[out_idx] != dummy_in[in_idx]) {
                            perm_pass = 0;
                        }
                    }
                }
        ...
        if (perm_pass) {
            printf("  ✅ Permute values verified.\n");
        } else {
